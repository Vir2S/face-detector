...
### ```Feature Branches (feature/*)``` — Used to develop new features or make changes. Each feature has its own branch and is created from dev. 

To create: ```git checkout -b feature/task-id_feature_name dev```

To merge: After completing the feature, merge it back into dev:

```git checkout dev```,

```git merge feature/task-id_feature_name```.

Then, delete the feature branch:

```git branch -d feature/task-id_feature_name```

Example: ```feature/FD-77_registration```

### ```Bugfix Branches (bugfix/*)``` — Used to fix bugs. Each bugfix has its own branch and is created from develop. 

To create: ```git checkout -b bugfix/task-id_bugfix_name dev```

To merge: After completing the bug fixing, merge it back into dev:

```git checkout dev```,

```git merge bugfix/task-id_bugfix_name```.

Then, delete the bugfix branch:

```git branch -d bugfix/task-id_bugfix_name```

Example: ```bugfix/FD-77_registration```

### ```Hotfix Branches (hotfix/*)``` — Used for urgent bug fixes in production. Created from dev when a critical issue needs to be resolved.

To create:

```git checkout -b hotfix/fix-name dev```

To merge: After fixing the issue, merge it into dev:
...
### Feature Development:
Developers create feature/* branches from dev, work on features, commit changes, push changes, create PR and after approve merge them back into dev.

### Bug Fixing:
Developers create bugfix/* branches from dev, work on bug, commit changes, push changes, create PR and after approve merge them back into dev.

### Hotfixes:
If a bug appears in production, a hotfix/* branch is created from master, the issue is fixed, commited, PR created, approved and the branch is merged into both master and dev.
...
This GitHub Action works for all new branches and for branches that contain the `.github/workflow/lint.yaml` file.
